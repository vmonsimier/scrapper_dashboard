...
@csrf_exempt
def scrapper_logs(request):
    """
    List all scrapper logs
    """
    if request.method == 'GET':
        scrapper_logs = ScrapperLogs.objects.all()
        serializer = serializers.ModelSerializer(scrapper_logs, many=True)
        return JsonResponse(serializer.data, safe=False)

    elif request.method == 'POST':
        data = JSONParser().parse(request)
        serializer = ScrapperLogsSerializer(data=data)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(serializer.data, status=201)
        return JsonResponse(serializer.errors, status=400)
...
@csrf_exempt
def scrapper_logs_detail(request, pk):
    """
    Retrieve, update or delete a code snippet.
    """
    try:
        scrapper_log = ScrapperLogs.objects.get(pk=pk)
    except ScrapperLogs.DoesNotExist:
        return HttpResponse(status=404)

    if request.method == 'GET':
        serializer = ScrapperLogsSerializer(scrapper_log)
        return JsonResponse(serializer.data)

    elif request.method == 'PUT':
        data = JSONParser().parse(request)
        serializer = ScrapperLogsSerializer(scrapper_log, data=data)
        if serializer.is_valid():
            serializer.save()
            return JsonResponse(serializer.data)
        return JsonResponse(serializer.errors, status=400)
...
def files(request):
    ...
    if request.method == 'GET':
        ...
        serializer = serializers.ModelSerializer(files, many=True)
        return JsonResponse(serializer.data, safe=False)
